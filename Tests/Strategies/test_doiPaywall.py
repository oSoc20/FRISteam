...
class TestDoiPaywall(unittest.TestCase):
    ...
    def test_add_doi_object(self):
        ...
        self.assertEqual(type(add_doi_object(publication_doi_url)), type(doi_obj))

    def test_extract_doi_from_url(self):
        self.assertRegex(publication_doi_url,'^https:\/\/doi.org\/([a-zA-Z0-9_.\/])*$')
        self.assertRegex(extract_doi_from_url(publication_doi_url),'^([a-zA-Z0-9_.\/])*$')
